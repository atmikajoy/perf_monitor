...
std::string pdh_error_string(DWORD error_code)
{
	static auto lib = LoadLibrary("pdh.dll");
	return {};
}
...
BOOL CPerfMon::UpdateValue(PPDHCOUNTERSTRUCT pCounter)
{
	...
	auto res = PdhGetFormattedCounterValue(pCounter->hCounter, PDH_FMT_DOUBLE, NULL, &pdhFormattedValue);
	...
	const double value = pdhFormattedValue.doubleValue;

	 pCounter->lValue = value;
	return true;
}
...
BOOL CPerfMon::GetStatistics(double* nMin, double* nMax, double* nMean, int nIndex)
{
	PDH_STATISTICS pdhStats;
	PPDHCOUNTERSTRUCT pCounter = GetCounterStruct(nIndex);
	if (!pCounter) return false;

	if (PdhComputeCounterStatistics(pCounter->hCounter, PDH_FMT_DOUBLE, pCounter->nOldestIndex, 
		pCounter->nRawCount, pCounter->a_RawValue, &pdhStats) != ERROR_SUCCESS)
		return false;
