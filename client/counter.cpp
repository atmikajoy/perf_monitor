...
namespace perf_monitor
{
	...
	std::string counter::to_string_brief() const
	{
		std::ostringstream stm;

		//std::string n = name;
		//std::string d = counter_names::description(name, true);
		//std::cout << "name: " << n << "  descr: " << d << '\n';

		stm << counter_names::description(name,true) << '\n' << std::setw(16) << source << " : " 
			<< std::setw(24) << str_time(time) <<  " : " ;
